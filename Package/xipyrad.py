...
def run_ipyrad_process(cluster_name, log, function=None):
    ...
    # build the ipyrad barcodes file
    if OK:
        if data_demultiplexed == 'NO':
            log.write(f'{xlib.get_separator()}\n')
            log.write('Building the file {0} ...\n'.format(get_ipyrad_barcodes_file()))
            (OK, error_list) = build_ipyrad_barcodes_file()
            if OK:
                log.write('The file is built.\n')
            if not OK:
                log.write('*** ERROR: The file could not be built.\n')

    # upload the ipyrad barcodes file
    if OK:
        if data_demultiplexed == 'NO':
            log.write(f'{xlib.get_separator()}\n')
            log.write('Uploading the file {0} to the directory {1} of the master ...\n'.format(get_ipyrad_barcodes_file(), current_run_dir))
            cluster_path = '{0}/{1}'.format(current_run_dir, os.path.basename(get_ipyrad_barcodes_file()))
            (OK, error_list) = xssh.put_file(sftp_client, get_ipyrad_barcodes_file(), cluster_path)
            if OK:
...
def build_ipyrad_barcodes_file():
    '''
    Build the ipurad barcodes file from individual file.
    '''
    ...
    try:
        barcode_file_id = open(get_ipyrad_barcodes_file(), mode='w', encoding='iso-8859-1', newline='\n')
    except Exception as e:
        error_list.append('*** ERROR: The file {0} can not be opened.'.format(get_ipyrad_barcodes_file()))
        OK = False
    ...
    try:
        ...
        with open(get_ipyrad_process_script(), mode='w', encoding='iso-8859-1', newline='\n') as script_file_id:
            ...
            if data_demultiplexed == 'NO':
                ...
                script_file_id.write( '{0}\n'.format('    sed -i "/$PARAMETRO_03/c\\{0}/{1}   $PARAMETRO_03" {2}'.format(current_run_dir, os.path.basename(get_ipyrad_barcodes_file()), parameter_file)))
                script_file_id.write( '{0}\n'.format('    RC=$?'))
                script_file_id.write( '{0}\n'.format('    if [ $RC -ne 0 ]; then manage_error sed $RC; fi'))
            ...
            script_file_id.write( '{0}\n'.format('    SUBJECT="{0}: {1} process"'.format(xlib.get_project_name(), xlib.get_ipyrad_name())))
            script_file_id.write( '{0}\n'.format('    MESSAGE="{0}"'.format(xlib.get_mail_message_ok(xlib.get_ipyrad_name(), cluster_name))))
            ...
            script_file_id.write( '{0}\n'.format('    RECIPIENT={0}'.format(xconfiguration.get_contact_data())))
            script_file_id.write( '{0}\n'.format('    SUBJECT="{0}: {1} process"'.format(xlib.get_project_name(), xlib.get_ipyrad_name())))
            script_file_id.write( '{0}\n'.format('    MESSAGE="{0}"'.format(xlib.get_mail_message_wrong(xlib.get_ipyrad_name(), cluster_name))))
            script_file_id.write( '    mail --append "Content-type: text/html;" --append "FROM:root@NGScloud2" --subject="$SUBJECT" "$RECIPIENT" <<< "$MESSAGE"\n')
