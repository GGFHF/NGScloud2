...
def run_htseq_count_process(cluster_name, log, function=None):
    ...
    if OK:
        (OK, error_list, is_installed) = xbioinfoapp.is_installed_bioconda_package(xlib.get_htseq_bioconda_code(), cluster_name, True, ssh_client)
        if OK:
            if not is_installed:
...
def build_htseq_count_process_script(cluster_name, current_run_dir):
    ...
    try:
        ...
        with open(get_htseq_count_process_script(), mode='w', encoding='iso-8859-1', newline='\n') as script_file_id:
            ...
            script_file_id.write( '{0}\n'.format('HTSEQ_PATH={0}/{1}/envs/{2}/bin'.format(xlib.get_cluster_app_dir(), xlib.get_miniconda3_name(), xlib.get_htseq_bioconda_code())))
            script_file_id.write( '{0}\n'.format('export PATH=$HTSEQ_PATH:$PATH'))
            script_file_id.write( '{0}\n'.format('cd {0}/{1}/bin'.format(xlib.get_cluster_app_dir(), xlib.get_miniconda3_name())))
            script_file_id.write( '{0}\n'.format('source activate {0}'.format(xlib.get_htseq_bioconda_code())))
            script_file_id.write( '#-------------------------------------------------------------------------------\n')
            ...
            script_file_id.write( '{0}\n'.format('    SUBJECT="{0}: {1} process"'.format(xlib.get_project_name(), xlib.get_htseq_count_name())))
            script_file_id.write( '{0}\n'.format('    MESSAGE="{0}"'.format(xlib.get_mail_message_ok(xlib.get_htseq_count_name(), cluster_name))))
            ...
            script_file_id.write( '{0}\n'.format('    RECIPIENT={0}'.format(xconfiguration.get_contact_data())))
            script_file_id.write( '{0}\n'.format('    SUBJECT="{0}: {1} process"'.format(xlib.get_project_name(), xlib.get_htseq_count_name())))
            script_file_id.write( '{0}\n'.format('    MESSAGE="{0}"'.format(xlib.get_mail_message_wrong(xlib.get_htseq_count_name(), cluster_name))))
            script_file_id.write( '    mail --append "Content-type: text/html;" --append "FROM:root@NGScloud2" --subject="$SUBJECT" "$RECIPIENT" <<< "$MESSAGE"\n')
